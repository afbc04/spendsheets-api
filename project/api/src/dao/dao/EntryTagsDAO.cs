...
namespace DAO {
    ...
    public class EntryTagsDAO {
        ...
        public async Task<Tag?> Get(long entryID, long tagID) {

            const string sql = @"SELECT 
                    tagId, name, description 
                FROM VEntryTagsByTag WHERE entryId = @entryID AND tagId = @tagID;";
        ...
        public async Task<long> Clear(long entryID) {

            const string sql = $"DELETE FROM EntryTags WHERE entryId = @entryID;";
            return await DAOUtils.Query(sql, async cmd => {
