...
namespace PacketHandlers {
    ...
    public static class PacketQueryValidatorFunctions {
        ...
        public static (PacketQueryValidatorObject,QueriesRequest?) validate_packet_query_fields(Dictionary<string, object> data, TemplateQuery requirements) {
        ...
        private static PacketQueryValidatorObject _validate_packet_body_fields_values(string key, object item, TemplateQueryItem requirements, PacketQueryValidatorObject pqv) {
        ...
        private static (PacketQueryValidatorPage?,List<string>) validate_packet_query_fields_page(Dictionary<string, object> data, bool has_pageable, Dictionary<string, TemplateQuerySortItem>? sort_opts) {
                ...
                if (Regex.IsMatch(sort,@"{(\w[\w_]*:-?1)(,(\w[\w_]*:-?1))*}") == false)
                    ...
                    foreach (string token in sorting_tokens) {
                        ...
                        if (sort_opts.ContainsKey(token_args[0].ToLower()) == false) 
                            ...
                            sort_list.Add(new QueryOrderItem(token_args[0],is_asc,sort_opts[token_args[0].ToLower()].is_case_insensitive,sort_opts[token_args[0].ToLower()].is_hidden));
